...
#      ---------- -----------  ---------------------------------
#      05/04/2017 Kunal Ghosh   Initial Code.
#
##########################################################################
...
#********************************************
	...
	if len(sys.argv[1:]) == 0 or len(sys.argv[1:]) != 4:
		print 'Usage Error: <py_job_name> <bucket_name> <file_name_with_full_path or dir path> <bucket_prefix> <one_file>'
		print 'Example: ./copy_from_ec2_to_s3.py fe-finr-da /mnt/data/Dev/bulk/mssql/prodcopy/account_plan_instrument/account_plan_instrument.csv Dev/bulk/mssql/prodcopy/account_plan_instrument/ y'
		print 'OR'
		print 'Example: ./copy_from_ec2_to_s3.py fe-finr-da /mnt/data/Dev/bulk/netezza/analytics/interaction_fact/ Dev/bulk/netezza/analytics/interaction_fact/ n'
		sys.exit(1)

	bucket_name=sys.argv[1]
	one_file=sys.argv[4].lower()
	f=lambda x: sys.argv[2] if(x == 'y') else [os.path.join(sys.argv[2],fn) for fn in next(os.walk(sys.argv[2]))[2]]
	file_name_with_full_path=f(one_file)
	if (one_file == 'y'):
		file_path = ''
		bname_file_path = ''
		py_job_name=py_job_name + '_' + '_'.join(os.path.basename(file_name_with_full_path).split('.'))
	else:
		file_path = sys.argv[2].rstrip('/') if sys.argv[2].endswith('/') else sys.argv[2]
		bname_file_path = os.path.basename(file_path)
		py_job_name=py_job_name + '_' + bname_file_path
...
#********************************************
	...
	print get_curr_date_time(), ': one_file :',one_file
	print get_curr_date_time(), ': file_path :',file_path
	print get_curr_date_time(), ': bname_file_path :',bname_file_path
...
# Check if directory is present # one block here
