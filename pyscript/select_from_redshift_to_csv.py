...
##########################################################################
#
#      PROGRAM: select_from_redshift_to_csv.py
#      PURPOSE: Extract tables from redshift to csv
#
#                                  ****** NOTE ******
#
#                                  ****** NOTE ******
...
#      ---------- -----------  ---------------------------------
#      04/12/2017 Kunal Ghosh   Initial Code.
#
##########################################################################
...
#********************************************
# Setting up the redshift logging 
...
#	dblogfile=py_job_name + '_redshift_py_connect_' + subpname + '_' + pcurrent_date + '.log'
...
#		msg={}
#		email_body = py_job_name + ' job has failed at ' + get_curr_date_time() + ' trying to set up logging for redshift for job_name.' + job_name
...
#********************************************
		email_subject = 'Error! ' + py_job_name
		msg={}
		email_body = py_job_name + ' job has failed at ' + get_curr_date_time() + ' trying to connect to Redshift for subjob ' + subpname + '.'
		msg['default'] = email_body
		msg_json=json.loads(json.dumps(msg))
		print get_curr_date_time(), ': email_subject :',email_subject
		print get_curr_date_time(), ': email_body :',email_body
...
#		for res in curr:
		...
		print get_curr_date_time(), ' Failed extracting data from redshift.'
		email_subject = 'Error! ' + py_job_name
		msg={}
		email_body = py_job_name + ' job has failed at ' + get_curr_date_time() + ' trying to run ' + sql_line + ' against redshift for subjob ' + subpname + '.'
		msg['default'] = email_body
		msg_json=json.loads(json.dumps(msg))
		print get_curr_date_time(), ': email_subject :',email_subject
		print get_curr_date_time(), ': email_body :',email_body
...
#********************************************
	...
	if len(sys.argv[1:]) == 0 or len(sys.argv[1:]) != 4:
		...
		print 'Example: ./select_from_redshift_to_csv.py user_segments /mnt/data/Dev/bulk/redshift/prodcopy/user_segments/ /mnt/project/di_de/sql/redshift_prodcopy_select_user_segments.sql y'
		sys.exit(1)
...
#********************************************
# Parallely extract data from redshift table
#********************************************
	sql_file=open(sql_file_with_path,'r')
	jobs=[]
	count=0
