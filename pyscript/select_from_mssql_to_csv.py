...
#      PURPOSE: Extract tables from mssql to csv
#
#                                  ****** NOTE ******
#
#                                  ****** NOTE ******
...
#	dblogfile=py_job_name + '_mssql_py_connect_' + subpname + '_' + pcurrent_date + '.log'
...
#		msg={}
#		email_body = py_job_name + ' job has failed at ' + get_curr_date_time() + ' trying to set up logging for mssql for job_name.' + job_name
...
#********************************************
	...
	cnx_string = 'DSN=%s;UID=%s;PWD=%s;DATABASE=%s;' % (mssql_dsn, mssql_user, mssql_password, mssql_database)
	try:
		cnx=pyodbc.connect(cnx_string)
	except:
		print get_curr_date_time(), ' Failed connecting to MSSQL.'
#********************************************
# Send job failure email
#********************************************
		email_subject = 'Error! ' + py_job_name
...
#		for res in curr:
		...
		print get_curr_date_time(), ' Failed extracting data from mssql.'
		email_subject = 'Error! ' + py_job_name
		msg={}
		email_body = py_job_name + ' job has failed at ' + get_curr_date_time() + ' trying to run ' + sql_line + ' against mssql for subjob ' + subpname + '.'
		msg['default'] = email_body
		msg_json=json.loads(json.dumps(msg))
		print get_curr_date_time(), ': email_subject :',email_subject
		print get_curr_date_time(), ': email_body :',email_body
...
#********************************************
	...
	mssql_dsn=common_config.mssql_dsn
	mssql_user=common_config.mssql_user
	mssql_password=common_config.mssql_password
...
#********************************************
	...
	print get_curr_date_time(), ': mssql_dsn :',mssql_dsn
	print get_curr_date_time(), ': mssql_user :',mssql_user
	print get_curr_date_time(), ': mssql_database :',mssql_database
